...
const editItem = ref<Major | null>(null)
const form = ref({
  name: '',
  description: '',
  icon: '',
...
function handleAdd() {
  editItem.value = null
  form.value = { name: '', description: '', icon: '', sort_order: 0, selectedIndustryIds: [] }
  errors.value = {}
  showModal.value = true
}

/* ── 打开编辑弹窗 ── */
async function handleEdit(item: Major) {
  editItem.value = item
  errors.value = {}
  // 加载该专业已关联的行业
  let selectedIds: number[] = []
  try {
    const res = await adminApi.getMajorIndustries(item.id)
    selectedIds = res.data.map((ind: Industry) => ind.id)
  } catch { /* ignore */ }
  ...
  form.value = {
    ...
    selectedIndustryIds: selectedIds,
  }
  showModal.value = true
}
...
function validate(): boolean {
  errors.value = {}
  if (!form.value.name.trim()) errors.value.name = '请输入专业名称'
...
async function handleSave() {
  ...
  try {
    if (editItem.value) {
      await adminApi.updateMajor(editItem.value.id, {
      ...
      // 更新关联行业
      await adminApi.setMajorIndustries(editItem.value.id, form.value.selectedIndustryIds)
    } else {
      const res = await adminApi.createMajor({
        name: form.value.name,
        description: form.value.description,
        icon: form.value.icon,
        sort_order: form.value.sort_order,
      ...
      if (form.value.selectedIndustryIds.length > 0) {
        await adminApi.setMajorIndustries(newId, form.value.selectedIndustryIds)
      }
    }
...
async function handleDelete(id: number) {
  if (!confirm('确定删除该专业？此操作不可恢复。')) return
  try {
...
/* ── 启用/禁用 ── */
async function handleToggleActive(item: Major) {
  await adminApi.updateMajor(item.id, { is_active: !item.is_active })
  loadData()
}
  ...
  <div>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="ef-overlay" @click="handleBackdropClick">
        <div class="ef-dialog">
          <div class="ef-header">
            <h2 class="ef-title">{{ editItem ? '编辑专业' : '新增专业' }}</h2>
          ...
          <form @submit.prevent="handleSave" class="ef-body" novalidate>
            ...
            <div class="ef-field ef-field--full">
              <label class="ef-label">专业名称<span class="ef-required">*</span></label>
              <input
                ref="nameRef"
                v-model="form.name"
                type="text"
            ...
            <div class="ef-field ef-field--full">
              ...
              <textarea v-model="form.description" rows="3" class="input-macos ef-textarea" />
            </div>
            ...
            <div class="ef-field-row">
              <div class="ef-field">
                ...
                <input v-model.number="form.sort_order" type="number" min="0" class="input-macos" />
              </div>
              <div class="ef-field">
                <label class="ef-label">图标</label>
                <input v-model="form.icon" type="text" class="input-macos" placeholder="图标名称或 URL" />
              </div>
            </div>
              ...
              <div class="max-h-48 overflow-y-auto border rounded-lg p-3 space-y-2 bg-gray-50">
                ...
                >
                  <input
                    type="checkbox"
                    :value="ind.id"
                    v-model="form.selectedIndustryIds"
                    class="rounded"
                  />
                  <span>{{ ind.name }}</span>
                </label>
